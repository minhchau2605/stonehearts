...
if (! function_exists('mfn_ID')) {
	function mfn_ID( $param = false )
	{
		...
		if( is_singular() ){
			// singulars template
			$portfolio_tmpl_id = mfn_single_post_ID('single-portfolio');
			$post_tmpl_id = mfn_single_post_ID('single-post');
...
if (!function_exists('mfn_shop_archive_tmpl') ) {
	function mfn_shop_archive_tmpl() {
		...
		if( !is_product_category() && !is_product_tag() && !is_shop() ) return false;
	...
	function mfn_breadcrumbs($params = false)
	{
		...
		if ( is_front_page() || is_home() ) {
			...
			if( is_product() ){
				...
				if( !empty( wc_get_page_id( 'shop' ) ) ) $breadcrumbs[] = '<a href="'. esc_url(get_permalink(wc_get_page_id( 'shop' ))) .'">'. esc_html(get_the_title(wc_get_page_id( 'shop' ))) .'</a>';
				if( !empty( $p_terms[0]->term_id ) ) $breadcrumbs[] = '<a href="'. esc_url(get_term_link($p_terms[0]->term_id)) .'">'. esc_html($p_terms[0]->name) .'</a>';
